...
# HDF5 I/O Benchmark Execution Script with Darshan Logging
# This script runs IOR with HDF5 interface and Darshan profiling
...
TEST_FILE="${TEST_FILE:-$LOG_DIR/ior_hdf5_testfile.h5}"
SEGMENT_COUNT="${SEGMENT_COUNT:-1}"
COLLECTIVE_METADATA="${COLLECTIVE_METADATA:-1}"
...
echo "=== Running HDF5 I/O Benchmark with Darshan Logging ==="
...
echo "  Collective metadata: $COLLECTIVE_METADATA"
...
IOR_CONFIG="$CONFIG_DIR/ior_hdf5_${RUN_ID}.conf"
cat > "$IOR_CONFIG" << EOF
# IOR Configuration for HDF5 I/O Test
# Generated on $(date)

# Test parameters
-a HDF5                    # Use HDF5 interface
...
--hdf5.collectiveMetadata=$COLLECTIVE_METADATA
--hdf5.chunkSize=$CHUNK_SIZE
--hdf5.setAlignment=1m
--hdf5.showHints=1
...
echo "Starting IOR HDF5 benchmark with Darshan profiling..."

# Run IOR with MPI
mpirun -np "$NUM_TASKS" \
    --oversubscribe \
    ior -f "$IOR_CONFIG" \
    2>&1 | tee "$LOG_DIR/ior_hdf5_${RUN_ID}.out"

echo "IOR HDF5 benchmark completed."
...
if [ -n "$DARSHAN_LOG" ] && [ -f "$DARSHAN_LOG" ]; then
    ...
    darshan-parser "$DARSHAN_LOG" > "$LOG_DIR/darshan_hdf5_${RUN_ID}.txt"
    
    # Generate summary
    echo "Generating Darshan summary..."
    darshan-job-summary.pl "$DARSHAN_LOG" --output "$LOG_DIR/darshan_hdf5_${RUN_ID}_summary.pdf" 2>/dev/null || echo "Note: PDF summary generation requires additional dependencies"
    
    # Extract HDF5 and POSIX counters
    echo "Extracting I/O counters..."
    python3 "$SCRIPT_DIR/extract_hdf5_counters.py" "$DARSHAN_LOG" "$LOG_DIR/hdf5_counters_${RUN_ID}.csv"
...
echo "=== HDF5 I/O Benchmark completed ==="
echo "Results saved to:"
echo "  IOR output: $LOG_DIR/ior_hdf5_${RUN_ID}.out"
if [ -n "$DARSHAN_LOG" ] && [ -f "$DARSHAN_LOG" ]; then
    echo "  Darshan log: $DARSHAN_LOG"
    echo "  Parsed log: $LOG_DIR/darshan_hdf5_${RUN_ID}.txt"
    echo "  I/O counters: $LOG_DIR/hdf5_counters_${RUN_ID}.csv"
fi
